...
void Surface::drawLine(int x0, int y0, int x1, int y1, uint32 color) {
	...
	if (format.bytesPerPixel == 1)
		...
		printf("Surface::drawLine: bytesPerPixel must be 1, 2, or 4");
	 */
	printf("drawLine\n");
}
...
void Surface::hLine(int x, int y, int x2, uint32 color) {
	...
	if (format.bytesPerPixel == 1) {
		...
		printf("Surface::hLine: bytesPerPixel must be 1, 2, or 4");
	}
}
...
void Surface::vLine(int x, int y, int y2, uint32 color) {
	...
	if (format.bytesPerPixel == 1) {
		...
		printf("Surface::vLine: bytesPerPixel must be 1, 2, or 4");
	}
}
...
void Surface::fillRect(Common::Rect r, uint32 color) {
	...
	if (format.bytesPerPixel == 2) {
		...
		printf("Surface::fillRect: bytesPerPixel must be 1, 2, or 4");
	}
...
void Surface::move(int dx, int dy, int height) {
	...
	if (format.bytesPerPixel != 1 && format.bytesPerPixel != 2 && format.bytesPerPixel != 4)
		printf("Surface::move: bytesPerPixel must be 1, 2, or 4");
...
void Surface::convertToInPlace(const PixelFormat &dstFormat, const byte *palette) {
	...
	if (format.bytesPerPixel == 0 || format.bytesPerPixel > 4)
		printf("Surface::convertToInPlace(): Can only convert from 1Bpp, 2Bpp, 3Bpp, and 4Bpp");

	if (dstFormat.bytesPerPixel != 2 && dstFormat.bytesPerPixel != 4)
		printf("Surface::convertToInPlace(): Can only convert to 2Bpp and 4Bpp");
	...
	if (dstFormat.bytesPerPixel > format.bytesPerPixel) {
		...
		if (!newPixels) {
			printf("Surface::convertToInPlace(): Out of memory");
		}
		pixels = newPixels;
	}
	...
	if (dstFormat.bytesPerPixel < format.bytesPerPixel) {
		void *const newPixels = realloc(pixels, w * h * dstFormat.bytesPerPixel);
		if (!newPixels) {
			printf("Surface::convertToInPlace(): Freeing memory failed");
		}
		pixels = newPixels;
	}
...
Graphics::Surface *Surface::convertTo(const PixelFormat &dstFormat, const byte *palette) const {
	...
	if (format.bytesPerPixel == 0 || format.bytesPerPixel > 4)
		printf("Surface::convertTo(): Can only convert from 1Bpp, 2Bpp, 3Bpp, and 4Bpp");

	if (dstFormat.bytesPerPixel != 2 && dstFormat.bytesPerPixel != 4)
		printf("Surface::convertTo(): Can only convert to 2Bpp and 4Bpp");
