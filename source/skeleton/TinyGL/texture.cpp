...
void glopTexImage2D(GLContext *c, GLParam *p) {
	...
	switch (format) {
		...
		case TGL_BGR:
			...
			break;
		default:
			printf("tglTexImage2D: Pixel format not handled.");
	}
	...
	if (target == TGL_TEXTURE_2D && level == 0 && components == 3 && border == 0 && pixels != NULL) {
		...
		printf("tglTexImage2D: combination of parameters not handled");
	}
...
error:
		printf("tglTexParameter: unsupported option");
	}
...
void glopPixelStore(GLContext *, GLParam *p) {
	...
	if (pname != TGL_UNPACK_ALIGNMENT || param != 1) {
		printf("tglPixelStore: unsupported option");
	}
}
