...
#ifdef __CROSS2D__
FuseZipData *initFuseZip(const char *program, unsigned char *romfs_data, int romfs_data_len,
...
FuseZipData *initFuseZip(const char *program, const char *fileName,
    ...
    zip_error_t zerror;
    zip_source *zip_src = zip_source_buffer_create((const void *) romfs_data, (size_t) romfs_data_len, 0, &zerror);
    if ((zip_file = zip_open_from_source(zip_src, flags, &zerror)) == nullptr) {
        fprintf(stderr, "initFuseZip: zip_open_from_source failed: %s\n", zip_error_strerror(&zerror));
