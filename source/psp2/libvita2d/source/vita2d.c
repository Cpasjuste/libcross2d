...
int vita2d_init_advanced(unsigned int temp_pool_size) {
    ...
    for (i = 0; i < DISPLAY_BUFFER_COUNT; i++) {
        ...
        err = sceGxmColorSurfaceInit(
                ...
                (MSAA_MODE == SCE_GXM_MULTISAMPLE_NONE) ? SCE_GXM_COLOR_SURFACE_SCALE_NONE
...
vita2d_shader *vita2d_create_shader(const SceGxmProgram *vertexProgramGxp, const SceGxmProgram *fragmentProgramGxp) {
    ...
    shader->paramPositionAttribute = (SceGxmProgramParameter *) sceGxmProgramFindParameterByName(vertexProgramGxp,
                                                                                                 "aPosition");
    DEBUG("aPosition sceGxmProgramFindParameterByName(): %p\n", shader->paramPositionAttribute);

    shader->paramTexcoordAttribute = (SceGxmProgramParameter *) sceGxmProgramFindParameterByName(vertexProgramGxp,
                                                                                                 "aTexcoord");
    ...
    textureVertexAttributes[0].format = SCE_GXM_ATTRIBUTE_FORMAT_F32;
    textureVertexAttributes[0].componentCount = 4; // (x, y, z, w)
    textureVertexAttributes[0].regIndex = (unsigned short) sceGxmProgramParameterGetResourceIndex(
            shader->paramPositionAttribute);
    /* u,v: 2 floats 32 bits */
    textureVertexAttributes[1].streamIndex = 0;
    textureVertexAttributes[1].offset = 16; // (x, y, z, w) * 4 = 16 bytes
    textureVertexAttributes[1].format = SCE_GXM_ATTRIBUTE_FORMAT_F32;
    textureVertexAttributes[1].componentCount = 2; // (u, v)
    textureVertexAttributes[1].regIndex = (unsigned short) sceGxmProgramParameterGetResourceIndex(
            shader->paramTexcoordAttribute);
    // 16 bit (short) indices
    textureVertexStreams[0].stride = sizeof(vita2d_texture_vertex);
    textureVertexStreams[0].indexSource = SCE_GXM_INDEX_SOURCE_INDEX_16BIT;
    ...
    shader->vertexInput.texture_size = (SceGxmProgramParameter *) sceGxmProgramFindParameterByName(vertexProgramGxp,
                                                                                                   "IN.texture_size");
    DEBUG("texture size sceGxmProgramFindParameterByName(): %p\n", shader->vertexInput.texture_size);

    shader->vertexInput.output_size = (SceGxmProgramParameter *) sceGxmProgramFindParameterByName(vertexProgramGxp,
                                                                                                  "IN.output_size");
    DEBUG("texture size sceGxmProgramFindParameterByName(): %p\n", shader->vertexInput.output_size);

    shader->vertexInput.video_size = (SceGxmProgramParameter *) sceGxmProgramFindParameterByName(vertexProgramGxp,
                                                                                                 "IN.video_size");
    DEBUG("texture size sceGxmProgramFindParameterByName(): %p\n", shader->vertexInput.video_size);

    shader->fragmentInput.texture_size = (SceGxmProgramParameter *) sceGxmProgramFindParameterByName(fragmentProgramGxp,
                                                                                                     "IN.texture_size");
    DEBUG("texture size sceGxmProgramFindParameterByName(): %p\n", shader->fragmentInput.texture_size);

    shader->fragmentInput.output_size = (SceGxmProgramParameter *) sceGxmProgramFindParameterByName(fragmentProgramGxp,
                                                                                                    "IN.output_size");
    DEBUG("texture size sceGxmProgramFindParameterByName(): %p\n", shader->fragmentInput.output_size);

    shader->fragmentInput.video_size = (SceGxmProgramParameter *) sceGxmProgramFindParameterByName(fragmentProgramGxp,
                                                                                                   "IN.video_size");
